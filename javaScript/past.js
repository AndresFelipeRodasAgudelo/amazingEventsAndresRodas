...
fetch("https://aulamindhub.github.io/amazing-api/events.json")
  ...
  .then(data => {
    ...
    document.querySelector(".padreChec").addEventListener("change", () => filtroPast(events));
    document.querySelector(".search1").addEventListener("input", () => filtroPast(events));
  })

  function filtroPast(events) {
    ...
    filtrar.length === 0? padreTarjetas.innerHTML = `
      ...
      </div>` : myModule.pintarPast(filtrar, padreTarjetas);    
}